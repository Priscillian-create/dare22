...
async function saveDataToSupabase(table, data, id = null) {
    ...
    if (table === 'inventory') {
        ...
        const { id, totalSales, totalTransactions, avgTransaction, dailySales, dailyTransactions } = dataForSupabase;
        dataForSupabase = {
            id,
            totalsales: totalSales,
            totaltransactions: totalTransactions,
            avgtransactions: avgTransaction,
            ...
            supabase.from('sales_data').select('*').eq('id', section).single().then(({ data, error }) => {
                if (error) {
                    ...
                    if (error.code === 'PGRST116') {
                        const initialSalesData = { id: section, totalsales: 0, totaltransactions: 0, avgtransactions: 0, dailysales: 0, dailytransactions: 0 };
                        supabase.from('sales_data').insert(initialSalesData).then(({ error: insertError }) => {
                            if (!insertError) {
                                salesData[section] = { totalSales: 0, totalTransactions: 0, avgTransaction: 0, topItem: '-', dailySales: 0, dailyTransactions: 0, profit: 0, profitMargin: 0 };
                                saveToLocalStorage(`salesData_${section}`, salesData[section]);
                                updateReports(section);
                                updateDepartmentStats(section);
                            }
                ...
                if (data) {
                    salesData[section] = {
                        ...
                        avgTransaction: data.avgtransactions || 0,
                        topItem: '-',
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const addItemButtons = document.querySelectorAll('.js-add-item-btn');
    addItemButtons.forEach(btn => {
        btn.addEventListener('click', () => {
            const section = btn.getAttribute('data-section');
            if (section) showAddItemModal(section);
        });
    });

    const addInventoryButtons = document.querySelectorAll('.js-add-inventory-btn');
    addInventoryButtons.forEach(btn => {
        btn.addEventListener('click', () => {
            const section = btn.getAttribute('data-section');
            if (section) showAddInventoryModal(section);
        });
    });

    const addSupplierButtons = document.querySelectorAll('.js-add-supplier-btn');
    addSupplierButtons.forEach(btn => {
        btn.addEventListener('click', () => {
            const section = btn.getAttribute('data-section');
            if (section) showAddSupplierModal(section);
        });
    });

    const addPurchaseOrderButtons = document.querySelectorAll('.js-add-purchase-order-btn');
    addPurchaseOrderButtons.forEach(btn => {
        btn.addEventListener('click', () => {
            const section = btn.getAttribute('data-section');
            if (section) showAddPurchaseOrderModal(section);
        });
    });
